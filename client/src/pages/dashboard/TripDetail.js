...
import {
  ...
  DialogContentText, DialogActions, Table, TableBody,
...
import { getTrip, updateTrip, deleteTrip, shareTripLink } from "../../redux/actions/tripActions";
...
const TripDetail = () => {
  ...
  const handleDeleteTrip = () => { dispatch(deleteTrip(id)); navigate("/dashboard/trips"); };
  ...
  const handleEditTrip = (e) => { e.preventDefault(); dispatch(updateTrip(id, editForm)); setEditOpen(false); };

  const handleShare = async () => {
    const data = await dispatch(shareTripLink(id));
    if (data) setShareData(data);
    setShareOpen(true);
  };

  const shareUrl = shareData?.shareToken
    ? `${window.location.origin}/trip/shared/${shareData.shareToken}`
    : currentTrip?.shareToken && currentTrip?.shareEnabled
    ? `${window.location.origin}/trip/shared/${currentTrip.shareToken}`
    : null;

  const tripImage = currentTrip?.images?.[0] || "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?fit=crop&w=1200";

  if (loading && !currentTrip) return <Box sx={{ display: "flex", justifyContent: "center", mt: 8 }}><CircularProgress /></Box>;
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="Share Trip">
            <IconButton onClick={handleShare} color="info" sx={{ bgcolor: "info.light" }}>
              <ShareIcon />
            </IconButton>
          </Tooltip>
      ...
      <Box sx={{ position: "relative", borderRadius: 4, overflow: "hidden", mb: 3, height: { xs: 200, md: 320 } }}>
        <Box component="img" src={tripImage} alt={currentTrip.destination} sx={{ width: "100%", height: "100%", objectFit: "cover" }} />
        <Box sx={{ position: "absolute", bottom: 0, left: 0, right: 0, p: 3, background: "linear-gradient(transparent, rgba(0,0,0,0.75))", color: "white" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1 }}>
            <PlaceIcon fontSize="small" />
            <Typography variant="h4" fontWeight={700}>{currentTrip.destination}</Typography>
          </Box>
          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
            <Chip label={currentTrip.status?.charAt(0).toUpperCase() + currentTrip.status?.slice(1)} color={STATUS_COLORS[currentTrip.status] || "default"} size="small" sx={{ fontWeight: 600 }} />
            <Chip icon={<DateRangeIcon style={{ color: "white" }} />} label={`${daysCount} days`} size="small" sx={{ bgcolor: "rgba(255,255,255,0.2)", color: "white" }} />
          </Box>
        </Box>
      </Box>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          <Grid container spacing={2} sx={{ mb: 3 }}>
            {[
              { label: "Start Date", color: "primary", value: new Date(currentTrip.startDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) },
              { label: "End Date", color: "secondary", value: new Date(currentTrip.endDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) },
              { label: "Budget", color: "success", value: `₹${(currentTrip.budget || 0).toLocaleString()}` },
              { label: "Spent", color: "warning", value: `₹${totalSpent.toLocaleString()}` },
          ...
          {currentTrip.budget > 0 && (
            <Paper elevation={0} sx={{ p: 3, borderRadius: 3, mb: 3, border: "1px solid", borderColor: "divider" }}>
              <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}>
                ...
                <Typography variant="subtitle2" fontWeight={700} color={budgetPercent > 90 ? "error.main" : "success.main"}>{budgetPercent.toFixed(1)}%</Typography>
              </Box>
              <LinearProgress variant="determinate" value={budgetPercent} color={budgetPercent > 90 ? "error" : budgetPercent > 70 ? "warning" : "success"} sx={{ height: 10, borderRadius: 5 }} />
        ...
        <Grid item xs={12} md={4}>
          <Paper elevation={0} sx={{ borderRadius: 3, border: "1px solid", borderColor: "divider" }}>
            <Box sx={{ p: 2.5, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <Button size="small" variant="contained" startIcon={<AddIcon />} onClick={() => setExpenseOpen(true)}>Add</Button>
            </Box>
            <Divider />
                    ...
                    expenses.map((e) => (
                      <TableRow key={e._id} hover>
                        ...
                        <TableCell align="right">
                          <IconButton size="small" color="error" onClick={() => dispatch(deleteExpense(e._id))}><DeleteIcon fontSize="small" /></IconButton>
                        </TableCell>
                      </TableRow>
                    ))
                  ) : (
        ...
        <DialogContent>
          <Box component="form" sx={{ mt: 1, display: "flex", flexDirection: "column", gap: 2 }}>
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <TextField fullWidth label="Amount (₹)" type="number" required value={expenseForm.amount} onChange={(e) => setExpenseForm({ ...expenseForm, amount: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth select label="Currency" value={expenseForm.currency} onChange={(e) => setExpenseForm({ ...expenseForm, currency: e.target.value })}>
                  {["INR","USD","EUR","GBP"].map((c) => <MenuItem key={c} value={c}>{c}</MenuItem>)}
                </TextField>
              </Grid>
            </Grid>
            <TextField fullWidth select label="Category" value={expenseForm.category} onChange={(e) => setExpenseForm({ ...expenseForm, category: e.target.value })}>
              {EXPENSE_CATEGORIES.map((c) => <MenuItem key={c} value={c}>{c}</MenuItem>)}
            </TextField>
            <TextField fullWidth label="Description / Note" value={expenseForm.description} onChange={(e) => setExpenseForm({ ...expenseForm, description: e.target.value })} />
            <TextField fullWidth type="date" label="Date" InputLabelProps={{ shrink: true }} value={expenseForm.date} onChange={(e) => setExpenseForm({ ...expenseForm, date: e.target.value })} />
          </Box>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
        ...
        <DialogContent>
          ...
          {shareUrl ? (
            <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
              ...
              <IconButton onClick={() => { navigator.clipboard.writeText(shareUrl); toast.success("Link copied!"); }}>
                <ContentCopyIcon />
              </IconButton>
            </Box>
          ) : (
            <Typography color="text.secondary">Generating link...</Typography>
          )}
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box component="form" sx={{ mt: 1, display: "flex", flexDirection: "column", gap: 2 }}>
            <TextField fullWidth label="Destination" value={editForm.destination || ""} onChange={(e) => setEditForm({ ...editForm, destination: e.target.value })} />
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <TextField fullWidth type="date" label="Start Date" InputLabelProps={{ shrink: true }} value={editForm.startDate || ""} onChange={(e) => setEditForm({ ...editForm, startDate: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth type="date" label="End Date" InputLabelProps={{ shrink: true }} value={editForm.endDate || ""} onChange={(e) => setEditForm({ ...editForm, endDate: e.target.value })} />
              </Grid>
            </Grid>
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <TextField fullWidth label="Budget (₹)" type="number" value={editForm.budget || ""} onChange={(e) => setEditForm({ ...editForm, budget: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth select label="Status" value={editForm.status || "planned"} onChange={(e) => setEditForm({ ...editForm, status: e.target.value })}>
            ...
            <TextField fullWidth multiline rows={3} label="Notes" value={editForm.description || ""} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} />
          </Box>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
